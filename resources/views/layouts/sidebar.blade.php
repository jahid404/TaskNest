...
<aside id="sidebar"
    class="fixed flex flex-col mt-0 top-0 px-5 left-0 bg-brand-25 dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-screen transition-all duration-300 ease-in-out z-99999 border-r border-brand-100 {{ $sidebarExpanded ?? true ? 'w-[290px]' : 'w-[90px]' }} {{ $isMobileOpen ?? false ? 'translate-x-0' : '-translate-x-full xl:translate-x-0' }}"
    x-data="{
        openSubmenus: {},
        init() {
        ...
        initializeActiveMenus() {
            const currentPath = '/' + '{{ $currentPath }}'.replace(/^\//, '');
    ...
    <!-- Logo Section -->
    <div class="pt-8 pb-7 hidden xl:flex"
        :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ?
        'xl:justify-center' :
        'justify-start'">
        <a href="/">
            <div x-cloak x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="text-2xl font-bold text-gray-900 dark:text-white">
                Task<span class="text-gray-400 dark:text-gray-500">Nest</span>
            </div>
            <div x-cloak x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                class="text-2xl font-bold text-gray-900 dark:text-white">
    ...
    <div class="flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar">
                                                    ...
                                                    <li>
                                                        <a href="{{ $subItem['path'] }}" class="menu-dropdown-item"
                                                            :class="isActive('{{ $subItem['path'] }}',
                                                                    {{ $subItem['exact'] ?? false ? 'true' : 'false' }}
                                                                ) ?
        ...
        <!-- Sidebar Widget -->
        <div x-cloak x-data
            x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen" x-transition
            class="mt-auto">
