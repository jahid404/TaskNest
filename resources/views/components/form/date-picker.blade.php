...
<div x-data="{
    ...
    init() {
        this.$nextTick(() => {
            const options = {
                ...
                defaultDate: {{ Js::from($value ?? $defaultDate) }},
                onReady: (selectedDates, dateStr, instance) => {
                    // Create footer container
                    const footer = document.createElement('div');
                    footer.className = 'flex items-center justify-between gap-2 p-2 px-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50';
                    
                    const todayBtn = document.createElement('button');
                    todayBtn.type = 'button';
                    todayBtn.className = 'text-[10px] font-bold uppercase tracking-wider text-brand-500 hover:text-brand-600 transition-colors';
                    ...
                    clearBtn.className = 'text-[10px] font-bold uppercase tracking-wider text-error-500 hover:text-error-600 transition-colors';
                ...
                onChange: (selectedDates, dateStr, instance) => {
                    this.notifyChange(dateStr, instance);
                }
            };
            
            this.flatpickrInstance = flatpickr(this.$refs.dateInput, options);

            // Watch for external changes (e.g. from parent Alpine store)
            this.$watch('$refs.dateInput.value', (val) => {
                if (this.flatpickrInstance && val !== this.flatpickrInstance.currentDateStr) {
                    this.flatpickrInstance.setDate(val, false);
                }
            });
        });
    },
    notifyChange(dateStr, instance) {
        ...
        // Trigger input event for x-model compatibility
    ...
    <div class="relative">
        <input
            ...
            class="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm transition focus:outline-hidden focus:ring-3 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 bg-transparent text-gray-800 {{ $error ? 'border-error-500 focus:border-error-500 focus:ring-error-500/10' : 'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700' }}"
            autocomplete="off"
            {{ $attributes }}
        />
